                            ...
                            bool *is_confocal)
{
    ...
    double& radiance_sum = local_array[threadIdx.x*MAX_THREADS_PER_BLOCK + threadIdx.y];
    radiance_sum = 0.0;
    ...
    for (uint cyi = 0; cyi < cam_y_loops; cyi++)
    {
        ...
        float distance_attenuation = camera_wall_distance * camera_wall_distance + 
                            voxel_camera_point_distance*voxel_camera_point_distance;
        if (!*is_confocal)
        {
            ...
            {   
                ...
                distance_attenuation += laser_wall_distance*laser_wall_distance;
                float laser_point_voxel_distance = distance(laser_wall_point, voxel_position);
                distance_attenuation += laser_point_voxel_distance * laser_point_voxel_distance;
                float total_distance = laser_wall_distance + laser_point_voxel_distance + voxel_camera_point_distance + camera_wall_distance;
                ...
                if (time_index < *T)
                {
                    ...
                    radiance_sum += transient_data[time_index * camera_grid_points[1] * camera_grid_points[0]
                        * laser_grid_points[1] * laser_grid_points[0] + cy * camera_grid_points[0] * laser_grid_points[1] * laser_grid_points[0] +
                        cx * laser_grid_points[1] * laser_grid_points[0] + ly * laser_grid_points[0] + lx] * distance_attenuation;
                }
            }
        }
        } else
