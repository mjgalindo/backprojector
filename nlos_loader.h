...
class NLOSData {
    ...
    array_type<T> load_transient_data_dataset(const H5::DataSet &dataset, 
        ...
        }
        {
            ...
            // Forcefully assume dimension 2 contains bounces to read them 1 by 1
            count[2]  = 1;
            dataspace.selectNone();
            for (const auto& b : bounces) 
            {
                offset[1] = b;
                dataspace.selectHyperslab(H5S_SELECT_OR, offset, count);
            }
        }

        std::cout << "Reading transient volume\n";
        dataset.read(buff, ptype, H5::DataSpace::ALL, dataspace);
